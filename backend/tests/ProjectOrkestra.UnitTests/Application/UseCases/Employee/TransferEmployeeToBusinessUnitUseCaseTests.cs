...
public class TransferEmployeeToBusinessUnitUseCaseTests
{
    ...
    public async Task ExecuteAsync_WhenSameOrganization_TransfersAndPersists()
    {
        ...
        var useCase = new TransferEmployeeToBusinessUnitUseCase(
            employeeRepository,
            businessUnitRepository
        );

        var organizationId = Guid.NewGuid();
        var currentBusinessUnit = new ProjectOrkestra.Domain.Entities.BusinessUnit(
            organizationId,
            "Loja 1",
            ValidCnpj,
            "Rua A, 1"
        );
        var targetBusinessUnit = new ProjectOrkestra.Domain.Entities.BusinessUnit(
            organizationId,
            "Loja 2",
            ValidCnpj,
            "Rua B, 2"
    ...
    public async Task ExecuteAsync_WhenTargetBusinessUnitDoesNotExist_ThrowsNotFoundException()
    {
        ...
        businessUnitRepository
            .GetByIdAsync(nonExistentBusinessUnitId)
