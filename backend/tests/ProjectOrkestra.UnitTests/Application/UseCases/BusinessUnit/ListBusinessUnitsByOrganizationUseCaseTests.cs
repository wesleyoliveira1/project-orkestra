...
public class ListBusinessUnitsByOrganizationUseCaseTests
{
    ...
    public async Task ExecuteAsync_WhenBusinessUnitsExist_ReturnsAllOfThem()
    {
        ...
        ListBusinessUnitsByOrganizationUseCase useCase = new ListBusinessUnitsByOrganizationUseCase(
            repository
        );

        Guid organizationId = Guid.NewGuid();
        List<ProjectOrkestra.Domain.Entities.BusinessUnit> businessUnits =
            new List<ProjectOrkestra.Domain.Entities.BusinessUnit>
            {
        ...
        repository
            .GetAllByOrganizationIdAsync(organizationId, Arg.Any<IEnumerable<BusinessUnitStatus>>())
